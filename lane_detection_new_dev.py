...
class HUD(object):
    ...
    def Lanes_detection(self,img,img_org, nwindows=50, margin=50, minpix = 1, draw_windows=False):
        ...
        for window in range(nwindows):
                ...
                win_y_low = binary_wraped.shape[0] - int((window+1)*window_height)
                win_y_high = binary_wraped.shape[0] -int(window*window_height)
                ...
                if draw_windows == True:
                    cv2.rectangle(binary_wraped,(win_xleft_low,win_y_low),(win_xleft_high,win_y_high),
                    (100,255,255), 3) 
                    cv2.rectangle(binary_wraped,(win_xright_low,win_y_low),(win_xright_high,win_y_high),
                    (100,255,255), 3)
        ...
        try:
                ...
                mid_ = np.mean(curves,axis=0)

                lanes =  (left_fit_, right_fit_)

                #curverad=self.get_curve(img, curves[0],curves[1])
                img_,_,x_y_coord = self.draw_lanes(img, curves[0], curves[1], mid_)
                ...
                img_2 = img_.astype(np.uint8)

                img_2_o = cv2.addWeighted(img_2_o, 0.3, img_2, 0.7, 0)
        ...
        return img_2, img_2_o, x_y_coord
    ...
    def wrap_image(self, img, inverse = False):
        ...
        if inverse:
            #print('step w 2')
            dst = np.float32([[img.shape[1]*(0.5 - mid_width/2), img.shape[0]*height_pct],[img.shape[1]*(0.5 + mid_width/2), img.shape[0]*height_pct],
    ...
    def draw_lanes(self, img, left_fit, right_fit, middle_fit):
        ...
        inv_perspective = cv2.putText(inv_perspective, 'Target Point', center_coordinates_t, font,
        fontScale, (0,255,255), thickness, cv2.LINE_AA)


        
        return inv_perspective, newwarp, center_coordinates
    ...
    def render(self, semantic_image, raw_image, depth_image, vehicle_location):
        if(raw_image is not None):
            ...
            mask_2 = np.zeros_like(raw_image)
            ...
            hexagon = np.array([
                                   [(int(raw_image.shape[1]*(0.5 - mid_width/2)), int(raw_image.shape[0]*height_pct)),
								    (int(raw_image.shape[1]*(0.5 + mid_width/2)), int(raw_image.shape[0]*height_pct)),
								    (int(raw_image.shape[1]*(0.5 + bot_width/2)), int(raw_image.shape[0]*bottom_trim)),
								    (int(raw_image.shape[1]*(0.5 - bot_width/2)), int(raw_image.shape[0]*bottom_trim))]
                                   ])
            
            #print(hexagon)
            mask_2 = cv2.fillPoly(mask_2, hexagon, 255)


            mask = cv2.bitwise_and(dilation.astype(float),mask_2.astype(float))
            #print('Before')
            Rot = np.array([[0,0,1],[1,0,0],[0,-1,0]])
            #out_img,raw_image,x_y_coords = self.Lanes_detection(mask, raw_image)
            #print('After')
            try:
                out_img,raw_image,x_y_coords = self.Lanes_detection(mask, raw_image)


                ## Calculations of the depth and the target point with respect to the world
                pixel= depth_image[x_y_coords[0],x_y_coords[1]]
                R=pixel[0]
                G=pixel[1]
                B=pixel[2]
                ...
                in_meters = 1000 * normalized  ## depth in meters


                #print(in_meters))
                #d_waypoint = [x_y_coords[0],x_y_coords[1],normalized]
                #d_waypoint_location = carla.Location(x=float(x_y_coords[0]),y=float(x_y_coords[1]),z=float(normalized))
                camera_waypoint = np.array([x_y_coords[0],x_y_coords[1],in_meters])
                
                world_waypoint = np.matmul(Rot,camera_waypoint)
                
                #print(world_waypoint)


                vehicle_pose = vehicle_location
                vy = vehicle_pose.y
                vz = vehicle_pose.z
                vx = vehicle_pose.x
                #print(vehicle_pose)
                ...
                #z=0.195892), 
                #d_waypoint_location = carla.Location(x=float(x_y_coords[0]+vx),y=in_meters+vy,z=0.195892)


                d_waypoint_location = carla.Location(x=float(world_waypoint[0])+vx,y=float(world_waypoint[1])+vy,z=0.195892)
                
                #print(d_waypoint_location)

                
                #print(d_waypoint_location)
                cv2.imshow('Segmented Lanes {}'.format(self.name), raw_image)
                #cv2.imshow('Triangle Mask {}'.format(self.name), mask)
                #cv2.imshow('Depth {}'.format(self.name), depth_image)
            ...
            #cv2.imshow('gray Image {}'.format(self.name), gray)
            #cv2.imshow('Canny Edges Image {}'.format(self.name), edges)

            #cv2.imshow('Segmented Lanes Image {}'.format(self.name), mask)
            #cv2.imshow('Canny Edges Image {}'.format(self.name), edges)
        ...
        return (key), mask,in_meters,d_waypoint_location
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.camera_manager_depth = CameraManager(self.vehicle, self.hud, args)
        self.camera_manager_depth.set_sensor(3, notify=False)   ## Logaritmic depth
    ...
    def render(self, display):
        ...
        if(self.hud is not None):
            self.hud.name = "raw"
            ret, segm,dep,way_point_location = self.hud.render(image_semantic, image, image_depth ,vehicle_location)



        return ret, image, segm, dep,way_point_location
...
# 200.
world = None
test1 = carla.Location(x=208.000000, y=195.000000, z=0.0)
test2 = carla.Location(x=208.000000, y=195.000000, z=0.5)
test3 = carla.Location(x=208.000000, y=195.000000, z=2)

try:
    world = World(client.get_world(), hud, args)   
    world.camera_manager.toggle_camera()
    world.camera_manager_semantic.toggle_camera()
    world.camera_manager_depth.toggle_camera()
    controller = KeyboardControl(world, args.autopilot)

    # test_point1 = client.get_world().debug.draw_string(test1, 'o', draw_shadow=False, 
	# 		color = carla.Color(r=255,g=0, b=0), life_time=2000, persistent_lines = True)

    # test_point2 = client.get_world().debug.draw_string(test2, 'o', draw_shadow=False, 
	# 		color = carla.Color(r=0,g=255, b=0), life_time=2000, persistent_lines = True)
    
    # test_point3 = client.get_world().debug.draw_string(test3, 'o', draw_shadow=False, 
	# 		color = carla.Color(r=0,g=0, b=255), life_time=2000, persistent_lines = True)


    tt = 0
    im_id = 0
    while ( True ):
        key, img, seg,dep,way_point_location= world.render(None)
        ##print(dep)
        #print(img.shape)
        #print(way_point_location)
        test_point1 = client.get_world().debug.draw_string(way_point_location, 'o', draw_shadow=False, 
			color = carla.Color(r=255,g=0, b=0), life_time=2000, persistent_lines = True)
        '''if tt >=100:
